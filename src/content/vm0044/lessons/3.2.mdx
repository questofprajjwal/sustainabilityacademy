...
<FormulaBox>
**Equation 9: Pyrolysis Process Emissions for Low Technology**

PE_P,p,y = sum over k and t of (F_e x GWP_CH4 x M_t,k,p,y)

Where:
- F_e = average methane emissions from producing one tonne of biochar (tCH4 per tonne of biochar). Source: Cornelissen et al. (2016), Table 3, based on kiln type.
- GWP_CH4 = global warming potential of methane. Use the value from the latest applicable VCS Standard. Current value = 28 (IPCC Fifth Assessment Report, AR5).
- M_t,k,p,y = mass of biochar type t for application type k produced at facility p in year y (tonnes, dry weight basis)

Default value when kiln type is unknown: F_e = 0.049 tCH4 per tonne of biochar produced. This default is based on traditional kilns, which are among the higher-emitting kiln types.
</FormulaBox>
