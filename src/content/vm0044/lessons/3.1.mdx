...
ER_PS,y = sum over t of [ (sum over k of CC_t,k,y x 44/12) - (sum over p of PE_PS,t,p,y) ]

Where:
*   ER_PS,y = GHG emission removals at the production stage in year y (tCO2e)
*   CC_t,k,y = organic carbon content on dry weight basis for biochar type t used for application type k in year y (tonnes of carbon)
*   PE_PS,t,p,y = project emissions at the production stage for biochar type t at facility p in year y (tCO2e)
*   44/12 = coefficient to convert tonnes of organic carbon to tCO2e (the molecular weight ratio of CO2 to carbon: one tonne of carbon produces 3.667 tonnes of CO2)
*   t = biochar type index
*   k = application type index
*   p = facility index
*   y = year
</FormulaBox>
...
Where:
*   M_t,k,p,y = mass on dry weight basis of biochar type t for application type k produced at facility p in year y (tonnes)
*   F_Cp,t,p = organic carbon content of biochar type t at production facility p, expressed per tonne of biochar on a dry weight basis (fraction, e.g. 0.77 means 77 percent carbon by dry mass). For high technology facilities: determined by laboratory material analysis.
*   PR_de,k = permanence adjustment factor due to decay of biochar for application type k (dimensionless, between 0 and 1)
</FormulaBox>
