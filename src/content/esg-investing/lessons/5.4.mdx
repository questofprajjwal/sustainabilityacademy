...
This asymmetry shapes how bond investors weight ESG factors:

- **Governance** tends to rank as the most important ESG pillar for credit investors. Fraud prevention, transparency, management integrity, and board oversight are directly connected to the risk that management makes decisions that impair the ability to service debt.
- **Environmental** factors matter to the extent they create financial liabilities — regulatory fines, remediation costs, stranded assets, or transition risks that impair cash flows. The *opportunity* side of environmental factors is less compelling for a bond investor: if a green acquisition creates value, the equity investor captures that upside; the bond investor mostly cares about whether the debt used to fund the acquisition remains serviceable.
- **Social** factors, particularly those related to human capital, supply chain integrity, and community relations, can create credit risk through reputational damage, regulatory action, or operational disruption.
...
- **Proxy vote** — bond investors typically do not have voting rights, limiting their ability to engage through shareholder mechanisms.
...
### Municipal Credit ESG Analysis

For **municipal bonds** — debt issued by regions, cities, or states — ESG analysis focuses on the issuer's governance and management practices (transparency, budgetary discipline, pension liabilities, contract governance) alongside local environmental and social factors that affect fiscal capacity. A municipality heavily exposed to physical climate risk, for example, faces future fiscal pressures from infrastructure damage and insurance costs.
...
For example, an issuer might commit to reducing scope 1 and scope 2 emissions by 30% by a specified date. If the target is not met, the coupon on the bond steps up by a defined margin (e.g. 25 basis points). If the target is met, the coupon does not step up.
