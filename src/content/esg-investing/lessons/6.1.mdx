...
The **Black-Litterman (BLM)** model offers a more robust alternative. Rather than requiring explicit return estimates for each asset class, BLM starts from global market equilibrium weights and allows the analyst to overlay specific "views." This makes it more naturally suited to incorporating ESG insights — for example, a view that carbon-intensive assets will underperform over the next decade — without requiring a precise quantitative estimate for each asset class return.

<HighlightBox>
The Black-Litterman model is considered particularly promising for ESG integration because it does not require precise return estimates for every asset class. Instead, it accepts directional views — such as "carbon-intensive assets face long-term headwinds" — and incorporates them alongside equilibrium assumptions.
</HighlightBox>
...
One concept increasingly discussed is the **Inevitable Policy Response (IPR)** — the idea that governments, faced with worsening climate damage, may suddenly implement aggressive climate policies in a "reflex reaction." The IPR scenario implies abrupt, disorderly transition risk. For SAA, this underscores the value of incorporating climate risk before it is forced upon the portfolio by policy shock.
