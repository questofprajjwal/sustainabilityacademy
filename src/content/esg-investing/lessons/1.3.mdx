...
<FormulaBox>
Total carbon emissions = Σᵢ (current value of investmentᵢ / issuer's market capitalisationᵢ) × issuer's Scope 1 and 2 emissionsᵢ
</FormulaBox>

**Weighted average carbon intensity** (relative):

<FormulaBox>
Weighted average carbon intensity = Σᵢ (current value of investmentᵢ / current portfolio value) × (issuer's Scope 1 and 2 emissionsᵢ / issuer's US$m of revenueᵢ)
</FormulaBox>

Carbon footprinting has important limitations. It typically captures only Scope 1 and 2 emissions, missing the often larger Scope 3 footprint. It can suffer from double-counting across portfolios. And it is a backward-looking metric — it tells you what a company emitted last year, not what its trajectory looks like.
