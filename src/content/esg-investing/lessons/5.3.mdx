...
The most significant practical challenge in ESG data is the low correlation between the ratings produced by different providers. Academic research (Berg et al.) found that the average correlation between ESG ratings from six major providers is approximately 0.54 (ranging from 0.38 to 0.71 depending on the pair). For the governance pillar, the average correlation is even lower — around 0.20.
...
**1. Scope divergence** — providers do not measure the same things. One provider might include a company's lobbying activities and political donations under its governance assessment; another might ignore them entirely. One might capture supply chain labour standards through satellite data; another only through self-disclosed policies.

**2. Measurement divergence** — even when measuring the same concept, providers use different metrics. One provider might measure workforce diversity by the percentage of women on the board; another might measure it across the entire employee base, producing very different scores for the same company.

**3. Aggregation divergence** — even when starting from similar underlying data, providers combine scores differently. One provider might weight climate change at 50% of the total ESG score; another at 20%. A company with strong climate credentials but weak social performance will be rated very differently under each approach.
