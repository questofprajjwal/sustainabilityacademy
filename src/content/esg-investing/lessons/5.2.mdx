...
Frameworks such as those developed by the Sustainability Accounting Standards Board (SASB) provide sector-specific materiality maps as a starting point. SASB identifies which environmental, social, and governance issues are most likely to be financially significant for companies in specific industries. For example:

- For **airlines**, climate change impact and employment quality are top-ranked.
- For **banks in developed markets**, community development, customer privacy, and board structure dominate.
- For **oil and gas**, climate change impact, resource management, and health and safety are primary.

However, these maps are guidance, not gospel. An investor may have good reasons to deviate — for example, if a company has an unusual business model that changes its exposure to a particular risk factor.
...
A useful way to think about what ESG analysis is actually capturing is through the lens of capital types. The International Integrated Reporting Council (IIRC) framework distinguishes between:
...
Many ESG factors represent intangible forms of capital that do not appear on a balance sheet but drive long-term value creation. A company's relationship with regulators (social capital), its employee culture (human capital), and its water usage efficiency (natural capital) are all ESG-assessable items. The analytical challenge is converting these qualitative assessments into financial implications.
...
Scorecards can also be used on private companies (where no external rating exists), sovereigns, and real estate or infrastructure assets. For sovereign bonds, for instance, environmental policy commitments, corruption levels, and governance effectiveness can all be scored and compared across countries.
...
- **ESG momentum tilts** — favouring companies with improving ESG scores, avoiding those with deteriorating profiles.
